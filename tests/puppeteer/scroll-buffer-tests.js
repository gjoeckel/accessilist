...
class ScrollBufferTests {
    ...
    /**
     * Test mychecklist.php scroll buffer initialization
     */
    async testMyChecklistScrollBuffer() {
        this.utils.logStep('Testing mychecklist.php scroll buffer initialization');

        try {
            await this.utils.navigateToPage(`${TEST_CONFIG.pages.checklist}?type=word&session=TEST1`);
            ...
            const bottomBufferValue = await this.page.evaluate(() => {
                return document.documentElement.style.getPropertyValue('--bottom-buffer');
            });

            // Verify scroll position starts at 0
            const initialScroll = await this.page.evaluate(() => window.scrollY);

            await this.utils.screenshot('mychecklist-scroll-buffer-init');

            if (scrollRestoration === 'manual' && topBuffer === '90px' && initialScroll === 0) {
                this.testResults.push({
                    test: 'MyChecklist Scroll Buffer Initialization',
                    status: 'PASSED',
                    details: `Top buffer: ${topBuffer}, Scroll: ${initialScroll}px, Bottom buffer: ${bottomBufferValue || 'dynamic'}`
                });
            } else {
                throw new Error(`Scroll buffer not configured correctly. Restoration: ${scrollRestoration}, Top buffer: ${topBuffer}, Scroll: ${initialScroll}`);
            }
    ...
    async testReportPagesScrollBuffer() {
        ...
        try {
            ...
            await this.page.goto(`${TEST_CONFIG.baseUrl}/list-report?session=${sessionKey}`);
            ...
            const bottomBufferValue = await this.page.evaluate(() => {
                return document.documentElement.style.getPropertyValue('--bottom-buffer-report');
            });
            ...
            if (topBuffer === '120px' && initialScroll === 130) {
                this.testResults.push({
                    ...
                    status: 'PASSED',
                    details: `Top buffer: ${topBuffer}, Initial scroll: ${initialScroll}px, Bottom buffer: ${bottomBufferValue || 'dynamic'}`
                });
            } else {
                throw new Error(`Report buffer not correct. Top: ${topBuffer}, Scroll: ${initialScroll}`);
            }
            ...
            if (systemwideTopBuffer === '120px' && systemwideScroll === 130) {
                ...
                throw new Error(`Systemwide buffer not correct. Top: ${systemwideTopBuffer}, Scroll: ${systemwideScroll}`);
            }
    ...
    async testDynamicBottomBuffer() {
        ...
        try {
            await this.utils.navigateToPage(`${TEST_CONFIG.pages.checklist}?type=word&session=BUFTEST`);
            await this.utils.waitForLoadingComplete();
            ...
            const bufferInfo = await this.page.evaluate(() => {
                const buffer = document.documentElement.style.getPropertyValue('--bottom-buffer');
            ...
            if (functionExists && bufferInfo.buffer) {
                ...
                throw new Error('Buffer calculation function not available or buffer not set');
            }
    ...
    async testAllButtonClickability() {
        ...
        try {
            ...
            if (!allButtonInfo.isClickable) {
                throw new Error(`All button blocked by element: ${allButtonInfo.elementAtCenter}`);
            }
            ...
            if (isActive && allButtonInfo.width === '30px' && allButtonInfo.height === '30px') {
                this.testResults.push({
                ...
                throw new Error(`All button not working correctly. Active: ${isActive}, Size: ${allButtonInfo.width}x${allButtonInfo.height}`);
            }
    ...
    async testCheckpointNavigation() {
        ...
        try {
            await this.utils.navigateToPage(`${TEST_CONFIG.pages.checklist}?type=word&session=NAVTEST`);
            await this.utils.waitForLoadingComplete();
            ...
            const checkpoint1State = await this.page.evaluate(() => {
                const btn1 = document.querySelector('.checkpoint-btn[data-checkpoint="checkpoint-1"]');
                const sections = document.querySelectorAll('.principle-section');
                const visibleSections = Array.from(sections).filter(s => s.style.display !== 'none').length;
            ...
            if (initialState.allActive &&
                checkpoint1State.button1Active && checkpoint1State.visibleSections === 1 &&
                finalState.allActive && finalState.visibleSections > 1) {
                this.testResults.push({
    ...
    async testPointerEventsPassThrough() {
        ...
        try {
            ...
            const pointerEventsInfo = await this.page.evaluate(() => {
                ...
                return {
                    container: container ? window.getComputedStyle(container).pointerEvents : 'not-found',
                    header: header ? window.getComputedStyle(header).pointerEvents : 'not-found',
                    filterGroup: filterGroup ? window.getComputedStyle(filterGroup).pointerEvents : 'not-found',
                    filterButton: filterButton ? window.getComputedStyle(filterButton).pointerEvents : 'not-found',
                    allButton: allButton ? window.getComputedStyle(allButton).pointerEvents : 'not-found'
                };
            });

            await this.utils.screenshot('pointer-events-config');

            if (pointerEventsInfo.container === 'none' &&
                ...
                throw new Error(`Pointer-events not configured correctly: ${JSON.stringify(pointerEventsInfo)}`);
            }
    ...
    async testBufferUpdateOnResize() {
        ...
        try {
            await this.utils.navigateToPage(`${TEST_CONFIG.pages.checklist}?type=word&session=RESIZE`);
            await this.utils.waitForLoadingComplete();
            await this.page.waitForTimeout(1000);
    ...
    async runAllTests() {
        ...
        // Generate report
        const report = this.utils.generateTestReport('scroll-buffer-tests', this.testResults);
