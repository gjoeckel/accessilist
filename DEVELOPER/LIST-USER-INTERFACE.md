...
**Process:** Load JSON template → Create checkpoint sections → Build tables (Task, Status, Notes, Actions columns) → Fire `contentBuilt` event
...
**Process:** Find checkpoint table → Create new row (taskId = currentRows.length) → Append row → Update scroll buffer → Trigger auto-save
