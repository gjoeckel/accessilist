...
class PlaywrightAPI {
    ...
    async interactWith(selector, action, options = {}) {
    ...
    async extractAndVerify(selector, options = {}) {
        const page = this.pages.get(this.currentBrowser);
        if (!page)
            throw new Error("Page not initialized - call navigate_and_wait first");
    ...
    async captureTestEvidence(options = {}) {
        const page = this.pages.get(this.currentBrowser);
        if (!page)
            throw new Error("Page not initialized - call navigate_and_wait first");
        const evidence = {
        ...
        if (options.screenshot) {
            const fullPage = typeof options.screenshot === "object"
...
const ExtractAndVerifySchema = z.object({
    ...
    attribute: z
        .string()
        .optional()
        .describe("Attribute to extract (text, value, html, or any attribute name)"),
...
const tools = [
    {
        ...
        description: "Navigate to a URL with smart waiting and multi-browser support. Auto-waits for page ready state. Supports Chromium, Firefox, and WebKit.",
        inputSchema: {
            type: "object",
    ...
    {
        name: "interact_with",
        description: "Interact with page elements (click, type, select, check). Auto-waits for element to be actionable (visible, stable, enabled). Includes built-in retry logic.",
        inputSchema: {
            type: "object",
    ...
    },
    {
        ...
        description: "Extract content from page elements with optional verification. Can extract text, values, HTML, or any attribute. Optionally verify against expected value.",
        inputSchema: {
            type: "object",
    ...
    },
    {
        ...
        description: "Capture comprehensive test evidence including screenshots, console logs, network logs, and page HTML. Useful for debugging and test reports.",
        inputSchema: {
            type: "object",
    ...
    try {
        switch (name) {
            ...
            case "interact_with": {
                ...
                const result = await playwrightAPI.interactWith(params.selector, params.action, params);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "extract_and_verify": {
                ...
                const result = await playwrightAPI.extractAndVerify(params.selector, params);
                return {
                    content: [
                        {
                            type: "text",
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                screenshot: result.screenshot
                                    ? `${result.screenshot.substring(0, 50)}... (${result.screenshotSize} bytes)`
                                    : undefined,
                            }, null, 2),
                        },
                    ],
