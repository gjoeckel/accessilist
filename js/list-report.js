...
export class UserReportManager {
  ...
  handleCheckpointClick(checkpoint) {
    ...
    this.checkpointButtons.forEach((btn) => {
      ...
      if (
        btnCheckpoint === String(checkpoint) ||
        (checkpoint === "all" && btnCheckpoint === "all")
      ) {
   ...
   */
  async updateTitle() {
    const displayName = await window.TypeManager.formatDisplayName(
      this.typeSlug
    );
  ...
  renderTable() {
    ...
    if (grouped.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="empty-state">No tasks found in this checklist</td></tr>';
      return;
    }
  ...
  applyCheckpointFilter() {
    ...
    this.allRows.forEach((item) => {
      ...
      const matchesCheckpoint =
        this.currentCheckpoint === "all" ||
        String(itemCheckpoint) === String(this.currentCheckpoint);
      const matchesStatus =
        this.currentFilter === "all" || this.currentFilter === status;
