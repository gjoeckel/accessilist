...
window.testBufferCalculation = async function () {
  ...
  try {
    ...
    const allButton = document.querySelector(
      '.checkpoint-btn[data-checkpoint="all"]'
    );
    if (allButton) {
      allButton.click();
      ...
      const allBuffer = getComputedStyle(
      ...
      results.all = {
        ...
        viewport: viewport,
        usedCalculation:
          totalHeight > viewport - topBuffer
            ? "large-content"
            : "small-content",
      ...
      console.log(
        `✓ Content > viewport? ${
          totalHeight > viewport - topBuffer ? "YES" : "NO"
        }`
      );
      console.log(
        `✓ Expected buffer: ${expectedBuffer}px (${results.all.usedCalculation})`
      );
    ...
    const checkpoint1Btn = document.querySelector(
    ...
    if (checkpoint1Btn) {
      ...
      const singleBuffer = getComputedStyle(
        document.documentElement
      ).getPropertyValue("--bottom-buffer");
      ...
      results.singleDefault = {
        ...
        viewport: viewport,
        usedCalculation:
          singleHeight > viewport - topBuffer
            ? "large-content"
            : "small-content",
      ...
      console.log(
        `✓ Content > viewport? ${
          singleHeight > viewport - topBuffer ? "YES" : "NO"
        }`
      );
      console.log(
        `✓ Expected buffer: ${expectedBuffer}px (${results.singleDefault.usedCalculation})`
      );
    ...
    if (addRowBtn && typeof window.handleAddPrincipleRow === "function") {
      ...
      const withRowBuffer = getComputedStyle(
        document.documentElement
      ).getPropertyValue("--bottom-buffer");
      ...
      results.singleWithRow = {
        ...
        viewport: viewport,
        usedCalculation:
          withRowHeight > viewport - topBuffer
            ? "large-content"
            : "small-content",
      ...
      console.log(
        `✓ Height increase: ${results.singleWithRow.heightIncrease}px`
      );
      console.log(`✓ Viewport height: ${viewport}px`);
      console.log(
        `✓ Content > viewport? ${
          withRowHeight > viewport - topBuffer ? "YES" : "NO"
        }`
      );
      console.log(
        `✓ Expected buffer: ${expectedBuffer}px (${results.singleWithRow.usedCalculation})`
      );
      ...
      const testRow = document.querySelector(
        ".checkpoint-1 tbody tr:last-child"
      );
      if (testRow) {
        testRow.remove();
